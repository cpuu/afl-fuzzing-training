...
# Comment out next line if your environment does not support browser opening from Makefile
# USE_BROWSER	= YES
BROWSER 	= firefox
SEPARATORS	= "\r\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\r\n"
...
VIMPORTS_DOT	= imports.dot
VIMPORTS_PNG	= imports.png
...
ifeq ($(USE_BROWSER), YES)
	$(BROWSER) afl_plot.html &
else
	@echo $(SEPARATORS)" Open file afl_plot.html in browser "$(SEPARATORS)
endif
...
ifeq ($(USE_BROWSER), YES)
	...
	@echo $(SEPARATORS)" Open file lcov_doc.html in browser "$(SEPARATORS)
endif
...
ifeq ($(USE_BROWSER), YES)
	...
	@echo $(SEPARATORS)" Open file dox_doc.html in browser "$(SEPARATORS)
endif
...
ifeq ($(USE_BROWSER), YES)
	...
	@echo $(SEPARATORS)" Open file "$(CWTRIAGE_OUT)" in browser "$(SEPARATORS)
endif
...
ifeq ($(USE_BROWSER), YES)
	...
	@echo $(SEPARATORS)" Open file afl-monitor_doc.html in browser "$(SEPARATORS)
endif
...
ifeq ($(USE_BROWSER), YES)
	...
	@echo $(SEPARATORS)" Open file "$(VISUALIZE_PNG)" in image viewer or browser "$(SEPARATORS)
endif
...
clean-results:
	rm -f $(ANGR_GEN_DICT) dox_doc.html lcov_doc.html afl_plot.html $(CWTRIAGE_OUT) $(CWDB_FILE) afl-monitor_doc.html .afl-monitor.state $(VISUALIZE_DOT) $(VISUALIZE_PNG) $(VIMPORTS_DOT) $(VIMPORTS_PNG)
...
parallel-visualize-imports:
	python $(SHARED_DIR)/afl_visualize_imports.py $(FINDINGS_DIR) $(VIMPORTS_DOT)
	dot -Tpng $(VIMPORTS_DOT) > $(VIMPORTS_PNG)
ifeq ($(USE_BROWSER), YES)
	$(BROWSER) $(VIMPORTS_PNG) &
else
	@echo $(SEPARATORS)" Open file "$(VIMPORTS_PNG)" in image viewer or browser "$(SEPARATORS)
endif
...
screen-std:
	screen -r afl_$(PROJECT)_slave_std-1
...
%.o_msan: %.c $(DEPS)
	AFL_USE_MSAN=1 $(CLANG) -fsanitize-memory-track-origins -fPIE -fno-omit-frame-pointer -g -c -o $@ $< $(CFLAGS)

$(EXEC_MSAN): $(OBJ_MSAN)
	AFL_USE_MSAN=1 $(CLANG) -fsanitize-memory-track-origins -fPIE -pie -fno-omit-frame-pointer -g -o $@ $^ $(LFLAGS)
